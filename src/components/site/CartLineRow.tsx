  ...
  return (
    <div className="flex gap-4 border-b border-ink/10 py-6 first:pt-0">
      <Link
        ...
        className={cn('relative block h-24 w-20 shrink-0 overflow-hidden bg-flax', outOfStock && 'opacity-40')}
      >
        {line.image && (
      ...
      <div className="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <Link href={`/products/${line.slug}`} className="text-ink hover:text-indigo">
            {line.name}
          </Link>
          <div className="mt-1">
            <Price cents={line.priceCents} className="text-sm text-ink/60" />
          </div>
          {outOfStock && <p className="mt-1 font-mono text-sm text-madder">Out of stock</p>}
          {lowStock && <p className="mt-1 font-mono text-sm text-madder">Only {line.stockQty} left</p>}
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="p-1.5 text-ink/60 hover:text-madder"
          >
